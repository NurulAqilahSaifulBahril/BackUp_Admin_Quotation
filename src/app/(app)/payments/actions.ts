 ...
 * linked_invoice can be bubble_id (text) or numeric string — both handled.
 */
...
export async function getFullyPaidInvoicesByAgent(month: number, year: number, search?: string) {
  try {
    ...
    for (const invoice of invoiceData) {
      const agentName = invoice.created_by_user_name || invoice.agent_name || 'Unknown User';
...
export async function verifyPayment(submittedPaymentId: number, adminId: string) {
  try {
    ...
    // Log payment verification into invoice_audit_log
    let actorName: string | undefined;
    let actorPhone: string | undefined;
    ...
    try {
      ...
      if (user) {
        ...
        actorPhone = user.phone || undefined;
  ...
  try {
    ...
    await logPaymentAudit({
      ...
      changes: changes.map((c) => {
        ...
        return match
          ? { field: match[1], before: match[2], after: match[3] }
          : { field: 'payment', before: null, after: c };
      }),
      actorName: user,
    });
