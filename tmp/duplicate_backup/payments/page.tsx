    ...
    if (viewMode === "by-agent" && groupedInvoices.length > 0) {
      ...
      groupedInvoices.forEach((group: any) => {
        ...
        group.invoices.forEach((inv: any) => {
          ...
          const fullDate = inv.full_payment_date ? formatDate(inv.full_payment_date) : "N/A";
          ...
          csvContent += `${agent},${inv.invoice_number},${customer},${firstDate},${fullDate},${amount},${commAmount},${description}\n`;
        });

        // Add a total row for the agent
        const totalEligible = group.total_eligible_amount ? group.total_eligible_amount.toFixed(2) : "0.00";
        csvContent += `"${group.agent_name} Total",,,,,${group.total_amount.toFixed(2)},${totalEligible},\n`;
        csvContent += ",,,,,,,\n"; // Empty row spacing
      });
    ...
    if (viewMode === "by-agent" && groupedInvoices.length > 0) {
      ...
      groupedInvoices.forEach((group: any) => {
        ...
        group.invoices.forEach((inv: any) => {
          ...
          tsvContent += `${agent}\t${invoiceNum}\t${customer}\t${firstDate}\t${fullDate}\t${amount}\t${commAmount}\t${description}\n`;
        });

        // Add a total row
        const totalEligible = group.total_eligible_amount ? group.total_eligible_amount.toFixed(2) : "0.00";
        tsvContent += `${clean(group.agent_name)} Total\t\t\t\t\t${group.total_amount.toFixed(2)}\t${totalEligible}\t\n`;
        tsvContent += "\t\t\t\t\t\t\t\n"; // Empty row
      });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex border-b border-secondary-200">
        ...
        <button
          onClick={() => setActiveTab("epp-costs")}
          className={cn(
            "px-6 py-3 text-sm font-medium border-b-2 transition-all flex items-center gap-2",
            activeTab === "epp-costs"
              ? "border-green-600 text-green-600 bg-green-50/50"
